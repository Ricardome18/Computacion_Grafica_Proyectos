...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model, glm::vec3(0.0f, ballPositionY, 0.0f)); // Aplica la traslación en el eje Y
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_N])
	{
		...
		if (!AnimBall)  // Si se desactiva la animación
		{
			ballPositionY = 0.0f;  // Restablece la posición de la pelota al origen (altura inicial)
		}
	}
}
//void Animation() {
